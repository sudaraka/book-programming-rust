...
/// bitmap. `pixel` is a pair indicating a particular pixel in that bitmap. The
/// `upper_left` and `lower_right` parameters are points on the complex plane
/// designating the area our bitmap covers.
...
fn main() {
  ...
  {
    let bannds: Vec<&mut [u8]> = pixels.chunks_mut(band_rows * bounds.0).collect();

    crossbeam::scope(|scope| {
      for (i, band) in bannds.into_iter().enumerate() {
        let top = band_rows * i;
